...
function eventToIcs(event: EsportEvent): Event | null {
  ...
  return new Event({
    ...
    desc: `${event.league.name} ${event.blockName} | ${event.match?.teams[0].name} ${event.match.teams[0].result ? event.match.teams[0].result.gameWins : ""} : ${event.match.teams[1].result ? event.match.teams[1].result.gameWins : ""} ${event.match?.teams[1].name}`,
    zone: "utc",
  });
}
